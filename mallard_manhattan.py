...
for num in (4, ):
    df = pl.scan_csv(
        f'data/mallard_et_al/MA_AUDIT_{num}_2020_03_25.tbl',
        separator='\t',
    ).select([
        (-pl.col('P-value').log10()).alias('neglog10_pval'),
        pl.col('ChrPosID').str.split_exact(':', 1).struct.field('field_0').alias('chr').cast(int),
        pl.col('ChrPosID').str.split_exact(':', 1).struct.field('field_1').alias('pos').cast(int)
    ]).filter(
        pl.col('neglog10_pval') > 3
    ).with_columns([
        pl.col('pos').alias('plot_pos'),
        pl.when(
            pl.col('neglog10_pval') < -np.log10(5e-8)
        ).then(
            pl.lit('#A9A9A9')
        ).otherwise(
            pl.lit('#00B8FF')
    ...
    manhattan_plot = bokeh.plotting.figure(
        ...
        height=400,
        title=f'Mallard Manhattan q{num}',
    ...
    bokeh.io.export_png(manhattan_plot, filename=f'data/mallard_et_al/mallard_q{num}_manhattan.png')
